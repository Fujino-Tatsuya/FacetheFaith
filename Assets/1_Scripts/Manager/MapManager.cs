...
public class MapManager : MonoBehaviour
{
    ...
    public void mapgener()// 만들어진 체스판에 특정 노드마다 스테이지 타입 부여
    {
        ...
        for (int path = 0; path < pathCount; path++)
        {
            ...
            //    type = StageNodeType.King;
            //    node.SetStageNode(type);
            //    piecePrefab = Instantiate(KingPrefab, new Vector3(0, 0, -5), transform.rotation);
            //    DontDestroyOnLoad(piecePrefab);
            //    node.currentPiece = piecePrefab;
            //    piecePrefab.GetComponent<Piece>().node = node;
            //}
    ...
    public void RevealNextNodesOnly(Node current)
    {
        ...
        //for (int y = 0; y < height; y++) // 잠시 주석처리
        //{
        //    for (int x = 0; x < width; x++)
        //    {
        //        node node = grid[x, y];
        ...
        //        //  같은 층의 다른 노드들은 무조건 꺼진다
        //        if (y == current.gridpos.y && node != current)
        //        {
        ...
        //        //  y+8의 스테이지 노드는 보이게
        //        bool isreveal8ahead = (y == ytarget) && node.isstagenode;

        //        if (!revealedpathnodes.contains(node) && !isreveal8ahead)
        //        {
        //            node.gameobject.setactive(false);
        //        }
        //    }
    ...
    //public void RevealPathToTarget(Node current, Node target)
    //{
    //    int curX = current.GridPos.x;
    //    int curY = current.GridPos.y;
    //    int targetX = target.GridPos.x;
    //    int targetY = target.GridPos.y;


    //    int xStep = (curX < targetX) ? 1 : -1; // x축 정렬 위한 방향 설정
    //    while (curX != targetX) // x축 이동
    //    {
    //        curX += xStep;
    //        if (curX < 0 || curX >= Width) break;

    //        Node node = grid[curX, curY];
    //        if (node != null)
    //        {
    //            node.gameObject.SetActive(true); // 이동하면서 노드 활성화
    //            revealedPathNodes.Add(node); // 저장
    //        }
    //    }

    //    // 2. y축 이동
    //    int yStep = 1; // y축 방향은 무조건 증가임
    //    while (curY != targetY)
    //    {
    //        curY += yStep;
    //        if (curY < 0 || curY >= Height) break;
    ...
    //            lr.positionCount = 2;
    //            lr.SetPosition(0, node.transform.position + Vector3.up * 0.001f); 
    //            lr.SetPosition(1, next.transform.position + Vector3.up * 0.001f);

    //            lr.startWidth = lr.endWidth = 0.05f;

    //            //  점선용 머티리얼 설정
    //            Texture2D tex = Resources.Load<Texture2D>("TestImage/DottedLineTexture");
    //            Material dottedMat = new Material(Shader.Find("Sprites/Default"));
    //            dottedMat.color = Color.black;

    //            if (tex != null)
    //            {
    //                dottedMat.mainTexture = tex;
    //                dottedMat.mainTextureScale = new Vector2(5f, 1f); // 점선 반복 강도 조절
    //            }
    //            //dottedMat.renderQueue = 1999;
    ...
    void SetInitialAlpha(GameObject root, float alpha)
    {
        ...
        foreach (var r in root.GetComponentsInChildren<Renderer>())
        {
            ...
            if (r.material.HasProperty("_Color"))
            {
                ...
                /*  (선택) 만약 Standard/Lit 셰이더로 투명도가 안 먹히면
                    r.material.SetFloat("_Mode", 2);          // Fade
                    r.material.EnableKeyword("_ALPHABLEND_ON");
                    r.material.renderQueue = 3000;            // 투명 큐
                */
            }
        }
    }
